const STORAGE_KEY = 'homeledger_state_v1';
...
function bulkDelete() {
  ...
  selectedTransactionIds.clear(); saveState(); renderAll(); toast('Transactions deleted', 'The selected records were removed from local storage.');
}
...
function exportBackup() {
  const blob = new Blob([JSON.stringify({ app: 'HomeLedger', version: 1, exportedAt: new Date().toISOString(), state }, null, 2)], { type: 'application/json' });
  downloadBlob(blob, `homeledger-backup-${new Date().toISOString().slice(0,10)}.json`);
  toast('Backup exported', 'Your household data was downloaded as a JSON file.');
}
...
async function importBackup(event) {
  ...
  try {
    ...
    saveState(); applyDisplaySettings(); renderAll(); switchView('dashboard'); toast('Backup restored', 'Your household data was loaded successfully.');
  } catch { toast('Could not restore backup', 'The selected JSON file is not a valid HomeLedger backup.'); }
  finally { event.target.value = ''; }
}

function clearAllData() {
  if (!confirm('Clear all transactions, imports, accounts, rules, and household settings stored in this browser? This cannot be undone unless you have a backup.')) return;
  state = structuredClone(DEFAULT_STATE); saveState(); selectedTransactionIds.clear(); hydrateAllSelects(); applyDisplaySettings(); renderAll(); switchView('dashboard'); toast('Local data cleared', 'HomeLedger was reset to its default state.');
}
